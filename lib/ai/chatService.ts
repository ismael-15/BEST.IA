const OUT_OF_SCOPE_TERMS = [
  'javascript', 'python', 'nextjs', 'react', 'css', 'html', 'sql', 'supabase',
  'hackear', 'hacking', 'contraseña', 'password', 'bitcoin', 'criptomonedas',
  'apuestas', 'casino', 'fútbol', 'real madrid', 'barcelona', 'política',
  'presidente', 'guerra', 'armas', 'programación', 'codigo', 'código',
];

const INJECTION_PATTERNS = [
  'ignora las instrucciones', 'ignore previous instructions', 'actúa como',
  'actua como', 'eres chatgpt', 'system prompt', 'muéstrame tu prompt',
  'muestrame tu prompt', 'olvida tus reglas', 'desactiva tus restricciones', 'jailbreak',
];

const CRISIS_TERMS = [
  'suicidio', 'suicidarme', 'matarme', 'quiero morir', 'no quiero vivir',
  'autolesión', 'autolesion', 'hacerme daño', 'hacerme dano', 'lastimarme',
  'desaparecer', 'no aguanto más', 'no aguanto mas', 'no vale la pena vivir',
...
export async function generateChatResponse(message: string): Promise<ChatServiceResult> {
...
`.trim();
  ...
  if (!response.ok) {
    ...
    return {
      ...
      detail: data?.error?.message || data?.message || 'No se pudo obtener respuesta del modelo.',
      status: 500,
    };
  }

  const parts = data?.candidates?.[0]?.content?.parts || [];
  const visiblePart = parts.find((part: any) => !part?.thought && typeof part?.text === 'string');
